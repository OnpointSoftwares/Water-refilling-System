...
$(document).ready(function() {
    ...
    $('#registration-frm').submit(function(e) {
            ...
            if ($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
                url: _base_url_ + 'classes/Login.php?f=registration',
                method: 'POST',
                data: $(this).serialize(),
                error: err => {
                    console.log(err)
                ...
                success: function(resp) {
                    if (resp) {
                        ...
                        if (resp.status == 'success') {
                            ...
                            var _frm = $('#login-frm')
                            var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Registration Error!! Try agaiin</div>"
                            _frm.prepend(_msg)
